...
class CLIPUpdater:
    ...
    def __call__(self, engine, batch):
        ...
        # feat_i = F.normalize(out["image_features"], dim=-1)
        # feat_t = F.normalize(out["text_features"], dim=-1)

        contrastive_loss = self.clip_loss(feat_i, feat_t, out["logit_scale"])
        total_loss = self.lambda_cont * contrastive_loss

        formula = f"lambda_cont×contrastive_loss"
        value_expr = f"{self.lambda_cont:.2f}×{contrastive_loss.item():.4f}"
        
        if self.teacher:
            loss_kd = self.loss_kd(images, texts, feat_i, feat_t)
            kd_weighted = self.lambda_kd * loss_kd
            total_loss += kd_weighted
            report.update({"loss_kd": loss_kd.detach().item()})
            formula += " + lambda_kd×loss_kd"
            value_expr += f" + {self.lambda_kd:.2f}×{loss_kd.item():.4f}"
        ...
        if current_step % 50 == 0:
            print(f"\nStep {current_step:>6d} | total_loss = {formula} = {value_expr} = {total_loss.item():.4f} (backprop)")
        # ==============================================
...
class RandRegUpdater(CLIPUpdater):
    ...
    def __call__(self, engine, batch):
        ...
        if self.share_random_feat:
            ...
            feat_tp = self.generate_random_feature(feat_t.size())  # 修复原代码笔误
        loss_feat = self.feature_loss_fn(feat_i, feat_ip) + self.feature_loss_fn(feat_t, feat_tp)
        feat_weighted = self.decay_rate * self.lambda_rand * loss_feat
        total_loss = loss_main + feat_weighted
        # 追加正则化的公式和数值
        formula += " + lambda_rand×decay_rate×loss_feat"
        value_expr += f" + {self.lambda_rand:.2f}×{self.decay_rate:.3f}×{loss_feat.item():.4f}"
